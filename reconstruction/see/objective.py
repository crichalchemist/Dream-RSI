...
def eq1_value(
    best: float | None, root_score: float, probes: int, rounds: int, beta1: float, beta2: float
) -> float:
...
def default_context(trace: Trace) -> GridPlanningContext:
    ...
    return GridPlanningContext(
        history=(),
        fallback_branch_count=b,
        fallback_refine_count=r,
        hard_max_branch_count=b,
        hard_max_refine_count=r,
        max_parallelism=trace.max_parallelism,
        trace_branch_count=b,
        trace_refine_count=r,
    )
...
def run_episode(
    ...
    *,
    ...
    return Episode(
        ...
        best=q.best_so_far,
        probes=probes,
        rounds=q.decision_rounds,
        effective_rounds=q.effective_sequential_rounds,
        work=probes / max(1, len(trace)),
        attainment=attainment(q.best_so_far, trace),
...
def beta_sweep(
    ...
    *,
    ...
    for beta in betas:
        eps = [
            run_episode(policy_cls({"beta": beta}), t, context_for(t), beta=beta, **kw)
            for t in traces
        ]
        executions += eps
        per_beta.append(
            {
                "beta": beta,
                "work": _mean(e.work for e in eps),
                "attainment": _mean(e.attainment for e in eps),
                "penalty": _mean(e.penalty for e in eps),
                "probes": _mean(e.probes for e in eps),
                "rounds": _mean(e.rounds for e in eps),
                "eq1": _mean(e.eq1 for e in eps),
            }
        )
    shipped = policy_cls(None)  # the baked-in default beta is what a live episode uses
    default_eps = [
        run_episode(policy_cls(None), t, context_for(t), beta=None, **kw) for t in traces
    ]
    ...
    report = {
        ...
        "errors": errors[:3],
        "pareto": {
            "reward": auc - lam * penalty if not errors else float("-inf"),
            "auc": auc,
        ...
        "default_episode": {
            "attainment": _mean(e.attainment for e in default_eps),
