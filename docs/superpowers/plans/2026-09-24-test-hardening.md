...
- Deviations recorded during execution (rulings; the spec is silent on each): (b) Task 4 — an evaluator result's `error` must be absent, `None` or a `str`; the brief's `or ""` form let falsy non-strings into the frozen `Observation.error: str | None` (fix fc79303). (c) Task 8 — CLAUDE.md says the digest is re-verified against the digest taken when it was archived, before scoring, not "the bytes that were scored"; `_archive` hashes before `_sweep` and the subprocess digest is never read. (d) Task 8 — recovery after an interrupted or refused iteration deletes `runs/iterNNNN`, `trace_pool/iterNNNN` and the aborted iteration's `policy_dev/history/r*_tNN_m*` directories, because the round counter is persisted only on success; the code-side guard is deferred to track C (extend `online()`'s guard with the first archive name; do not refuse inside `_archive`). (e) Task 8 — GAPS §7 item 2's "stale trees a killed run left behind" is narrowed to the agent-crash case (`live.py` removes only the program); Task 2's guard makes the wide reading unreachable. (f) Task 8 — the CLAUDE.md conventions bullet was edited although the brief listed it as unchanged; it is accurate against the guard and (d) touches it anyway. (g) Final review — the `_sweep` subprocess outlives a killed parent (`subprocess.run`, no process group); the recovery bullet warns about it, the `Popen` fix is track C. (h) Final review — one fix wave: a third round in `test_one_malformed_evaluator_result_fails_one_cell_not_the_batch` pinning a falsy non-string `error` and a missing `combined_score` (count stays 55), the orphan-sweep clause, and two one-word GAPS fixes (100086e, 2ab5387). (i) A stale helper name (`malformed_on_branch_1`) left by (h) is parked for track G.
