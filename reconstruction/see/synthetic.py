...
def synthetic_trace(
    ...
    length = [
        refine + 1 if rng.random() < 0.6 else rng.randint(2, refine + 1) for _ in range(branches)
    ]
        ...
        for b in range(branches):
            ...
            if r < env_rate:
                ...
                cell = Cell(
                    ...
                    seq,
                    0.0,
                    evaluated=kind != "compile_other",
                    valid=False,
                    fail_class=kind,
                    error=f"synthetic {kind} failure",
                    n_valid=0,
                    n_total=17,
                )
            else:
    ...
    return Trace(
        ...
        baseline,
        max_parallelism,
        trace_id=f"synthetic-{seed}",
        grid=(branches, refine),
        info={"generator": "see.synthetic", "seed": seed},
    )
