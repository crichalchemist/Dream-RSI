...
12. `install_signal_handlers()` also maps SIGHUP, leaving it alone when it was inherited ignored (a
    nohup launch): with the agents and the sweep in their own sessions they no longer
...
17. Spec §5 and its §3 rows were amended after the fix wave to describe the lifetime as built
    (Rulings 5, 7, 10, 14, 15 and 16: sliced wait, claim-based `terminate()`, pipes closed instead
    of drained, unconditional SIGKILL, `errors="replace"`, sweep spawn failure propagating), so the
    spec and the code agree; the rulings above remain the reasons for each departure.
18. The re-review of the fix wave found that the controller's own edit for Ruling 17 (dc0b275) had
    replaced the plan's closing paragraph with a second copy of the whole file; the plan was
    restored from a64b478 and the ruling inserted again. With it: the `_sweep` §3 row gained
    `errors="replace"`; "or setsid" was dropped from Ruling 12 and the `install_signal_handlers`
    docstring (setsid(1) does not ignore SIGHUP); `CommandAgent.__call__` sets its deadline inside
    the `try`, so nothing between the spawn and the `finally` can leave a registered child
    unkilled; GAPS.md's lifetime row names the unclaimed case Ruling 14 deferred.
